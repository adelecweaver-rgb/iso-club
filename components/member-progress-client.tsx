...
function dirColor(dir: "up" | "down" | "none", goodDir: "up" | "down"): string {
  if (dir === "none") return "#6B7B6E";
  return dir === goodDir ? "#4A7C59" : "#B84040";
}
...
export function MemberProgressClient({ payload }: { payload: DashboardPayload }) {
  ...
  const cardBg = (dir: "up" | "down" | "none", good: "up" | "down") => {
    ...
    return dir === good ? "rgba(74,124,89,0.04)" : "rgba(224,82,82,0.04)";
  };

  return (
    <main style={{ minHeight: "100vh", background: "#1C2B1E", color: "#edeae0", maxWidth: 800, margin: "0 auto", padding: "24px 20px" }}>

      {/* Back */}
      <div style={{ marginBottom: 24 }}>
        <Link href="/dashboard" style={{ fontSize: 12, color: "#6B7B6E", textDecoration: "none" }}>← Back to Dashboard</Link>
      </div>

      {/* ── Header ──────────────────────────────────────────────────────────── */}
      <div style={{ marginBottom: 32 }}>
        <div style={{ fontSize: 28, fontWeight: 800, color: "#edeae0", marginBottom: 6, fontFamily: "Georgia, serif" }}>Healthspan Progress</div>
        <div style={{ fontSize: 14, color: "#6B7B6E", marginBottom: 4 }}>Your body&apos;s story since joining Iso Club</div>
        {payload.memberSince && (
          <div style={{ fontSize: 12, color: "rgba(74,124,89,0.6)", fontWeight: 500 }}>Member since {payload.memberSince}</div>
        )}
      </div>

      {/* ── Section 1: Vitality Age ─────────────────────────────────────────── */}
      <section style={{ marginBottom: 32 }}>
        <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.16em", color: "#6B7B6E", marginBottom: 14 }}>Vitality Age</div>
        <div style={{ background: "linear-gradient(135deg, rgba(74,124,89,0.07) 0%, rgba(74,124,89,0.02) 100%)", border: "1px solid rgba(74,124,89,0.2)", borderRadius: 16, padding: "24px 28px" }}>
          {payload.vitalityAge.hasEnoughData && payload.vitalityAge.estimated !== null ? (
            <>
              <div style={{ display: "flex", alignItems: "flex-end", gap: 16, marginBottom: 10 }}>
                ...
                <div style={{ paddingBottom: 8, color: "#6B7B6E", fontSize: 13 }}>years old (functionally)</div>
              </div>
              {payload.vitalityAge.difference !== null && payload.vitalityAge.difference !== 0 && (
                <div style={{ fontSize: 15, color: payload.vitalityAge.difference > 0 ? "#4A7C59" : "#B84040", fontWeight: 500, marginBottom: payload.vitalityAge.trend !== null ? 4 : 0 }}>
              ...
              {payload.vitalityAge.trend !== null && payload.vitalityAge.trend !== 0 && (
                <div style={{ fontSize: 12, color: payload.vitalityAge.trend > 0 ? "#4A7C59" : "#B84040" }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 13, color: "#6B7B6E", margin: 0, lineHeight: 1.6 }}>
      ...
      <section style={{ marginBottom: 32 }}>
        <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.16em", color: "#6B7B6E", marginBottom: 14 }}>The 4 key metrics</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 12 }}>
          ...
          <div style={{ background: cardBg(gm.sinceJoiningDir, "up"), border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "18px 20px" }}>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#6B7B6E", marginBottom: 12 }}>Muscle</div>
            <div style={{ fontSize: 24, fontWeight: 800, color: "#edeae0", marginBottom: 12 }}>
              {payload.scan.leanMassLbs !== "--" ? `${payload.scan.leanMassLbs} lbs` : "—"}
              <span style={{ fontSize: 12, fontWeight: 400, color: "#6B7B6E", marginLeft: 6 }}>lean mass</span>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                <span style={{ color: "#6B7B6E" }}>Since joining</span>
                <span style={{ fontWeight: 600, color: dirColor(gm.sinceJoiningDir, "up") }}>{gm.sinceJoining}{dirArrow(gm.sinceJoiningDir)}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                <span style={{ color: "#6B7B6E" }}>Last month</span>
          ...
          <div style={{ background: cardBg(lf.sinceJoiningDir, "down"), border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "18px 20px" }}>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#6B7B6E", marginBottom: 12 }}>Body Fat</div>
            <div style={{ fontSize: 24, fontWeight: 800, color: "#edeae0", marginBottom: 12 }}>
          ...
          <div style={{ background: cardBg(ic.sinceJoiningDir, "up"), border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "18px 20px" }}>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#6B7B6E", marginBottom: 12 }}>Cardio Power</div>
            <div style={{ marginBottom: 12 }}>
              {peakPower > 0 && (
                <div style={{ fontSize: 22, fontWeight: 800, color: "#edeae0", lineHeight: 1.2 }}>{Math.round(peakPower)}W peak</div>
              )}
              {latestManp && (
                <div style={{ fontSize: 13, color: "#6B7B6E", marginTop: 2 }}>{latestManp.manp} MANP</div>
              )}
          ...
          <div style={{ background: cardBg(att.sinceJoiningDir, "up"), border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "18px 20px" }}>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#6B7B6E", marginBottom: 12 }}>Consistency</div>
            <div style={{ fontSize: 24, fontWeight: 800, color: "#edeae0", marginBottom: 12 }}>
              {totalSessions > 0 ? totalSessions : "—"}
              {totalSessions > 0 && <span style={{ fontSize: 13, fontWeight: 400, color: "#6B7B6E", marginLeft: 6 }}>total sessions</span>}
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                <span style={{ color: "#6B7B6E" }}>This month</span>
              ...
              {payload.longestStreakWeeks > 0 && (
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                  <span style={{ color: "#6B7B6E" }}>Longest streak</span>
                  <span style={{ fontWeight: 500, color: "#4A7C59" }}>{payload.longestStreakWeeks} week{payload.longestStreakWeeks !== 1 ? "s" : ""}</span>
                </div>
              )}
            </div>
      ...
      {(leanVals.some((v) => v !== null) || fatVals.some((v) => v !== null) || powerVals.some((v) => v !== null)) && (
        <section style={{ marginBottom: 32 }}>
          <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.16em", color: "#6B7B6E", marginBottom: 14 }}>Trends</div>
            ...
            {[
              ...
              const isGoodTrend =
                chart.first != null && chart.last != null
                  ...
                  : true;
              const lineColor = chart.first == null ? "#6B7B6E" : isGoodTrend ? "#4A7C59" : "#B84040";
              return (
                <div key={chart.label} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "14px 14px 10px" }}>
                  <div style={{ fontSize: 9, textTransform: "uppercase", letterSpacing: "0.12em", color: "#6B7B6E", marginBottom: 8 }}>{chart.label}</div>
                  {path ? (
                    <>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span style={{ fontSize: 9, color: "#6B7B6E" }}>
      ...
      {payload.milestones.length > 0 && (
        <section style={{ marginBottom: 32 }}>
          <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.16em", color: "#6B7B6E", marginBottom: 14 }}>Milestones</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
            {payload.milestones.map((m, i) => (
              <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 14, padding: "12px 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                ...
                <div style={{ fontSize: 11, color: "#6B7B6E", whiteSpace: "nowrap", flexShrink: 0 }}>{m.dateLabel}</div>
              </div>
            ))}
          </div>
        </section>
