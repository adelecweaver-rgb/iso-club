...
export async function POST(request: Request) {
  try {
    ...
    const supabase = createSupabaseAdminClient();
    if (!supabase) return NextResponse.json({ success: false, error: "Supabase unavailable." }, { status: 500 });
    const body = (await request.json()) as { goal_type?: string; is_active?: boolean };
    const goalType = asString(body.goal_type) as GoalType;
    if (!VALID_GOALS.includes(goalType)) {
      return NextResponse.json({ success: false, error: "Invalid goal_type." }, { status: 400 });
    }
    const isActive = body.is_active !== false;
    const res = await supabase.from("member_goals").upsert(
      { member_id: memberId, goal_type: goalType, is_active: isActive, set_by: memberId, updated_at: new Date().toISOString() },
