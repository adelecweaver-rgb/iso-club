...
export default async function CoachCapturePage() {
  ...
  const prototype = await loadPrototypeFromFiles(
    ["iso-club-ai-capture.html"],
    "Coach AI Data Capture",
  );
  ...
  if (!prototype.source) {
    return (
      <main className="shell">
        <div className="card">
          <h1 className="title">AI capture source missing</h1>
          <p className="muted">
            Add
            {" "}
            <code>iso-club-ai-capture.html</code>
            {" "}
            to the repository root to render this page design.
          </p>
          <Link className="btn" href="/coach/log">
            Back to staff log
          </Link>
        </div>
      </main>
