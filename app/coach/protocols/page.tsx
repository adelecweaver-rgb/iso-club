...
export default async function CoachProtocolsPage() {
  ...
  const prototype = await loadPrototypeFromFiles(
    ["iso-club-protocol-builder.html"],
    "Coach Protocol Builder",
  );
  ...
  if (!prototype.source) {
    return (
      <main className="shell">
        <div className="card">
          <h1 className="title">Protocol builder source missing</h1>
          <p className="muted">
            Add
            {" "}
            <code>iso-club-protocol-builder.html</code>
            {" "}
